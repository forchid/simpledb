...
public class BetterQueryPlanner implements QueryPlanner {
   ...
   public Plan createPlan(QueryData data, Transaction tx) {
     ...
     List<Plan> plans = new ArrayList<Plan>();
      ...
      // At each step, the lhs plan has the smallest RPB
      Plan p = plans.remove(0);
      for (Plan nextplan : plans) {
         double nextrpb = nextplan.recordsOutput() / nextplan.blocksAccessed();
         double prpb = p.recordsOutput() / p.blocksAccessed();
         if (nextrpb < prpb)
            p = new ProductPlan(nextplan, p);
         else
